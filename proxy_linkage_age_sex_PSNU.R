...
#import data
linkage_data <- ICPI_FactView_PSNU_IM_20170922_v2_1Zambia
...
linkage_data2 <-linkage_data %>%
  ...
  select(psnuuid,psnu,indicator,standardizeddisaggregate,age,sex,resultstatus,fy2017q1,fy2017q2,fy2017q3) %>% 
  group_by(psnuuid,psnu,indicator,age,sex,resultstatus,add=TRUE) %>% 
...
totalN <- ICPI_FactView_PSNU_IM_20170922_v2_1Zambia
...
totalN <- totalN %>% 
  ...
  select(psnuuid,psnu,indicator,standardizeddisaggregate,fy2017q1,fy2017q2,fy2017q3) %>% 
  group_by(psnuuid,psnu,indicator,standardizeddisaggregate,add=TRUE) %>% 
  summarize_at(vars(starts_with("fy2017q")),funs(sum(.,na.rm=TRUE))) %>% 
  ungroup() %>% 
...
#export
write.table(Linkage_Final, file="proxylinkage_PSNU_agesex_20170922.txt",  sep="\t")