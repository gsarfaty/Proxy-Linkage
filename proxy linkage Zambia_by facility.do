...
** WRANGLING **
	...
	*aggregate so there is only one obvervation per mechanism
		collapse (sum) fy2*, by(operatingunit psnu facility indicator)
...
** Linkage **
	...
	*reshape long to allow dataset to become a timeseries & transform time variable to date format
		egen id = group(operatingunit psnu facility indicator)
	...
	*reshape for calculation
		reshape wide fy, i(qtr operatingunit psnu facility) j(indicator, string)
	...
	* replace
	...
	export excel "Zambia_Proxy_Linkage by Facility.xlsx", firstrow(variables) replace
